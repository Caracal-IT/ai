...
async function loadInquirerPrompts() {
  try {
    ...
    if (isOptionalInquirerLoadError(requireError)) {
      return null;
    }
...
async function getInquirerPrompt(name) {
  if (!inquirerPrompts) {
    inquirerPrompts = await loadInquirerPrompts();
  }

  if (!inquirerPrompts) return null;
...
function createRadioPromptChoices(items, defaultKey) {
...
async function selectOne(rl, title, items, defaultKey) {
  ...
  const checkboxPrompt = await getInquirerPrompt('checkbox');
  const hasValidDefault = findDefaultIndex(items, defaultKey) !== null;
  if (checkboxPrompt) {
    const selectedValues = await checkboxPrompt({
      ...
      instructions: true,
      required: true,
      choices: createRadioPromptChoices(items, hasValidDefault ? defaultKey : null),
      validate(selected) {
        if (selected.length === 1) return true;
        return 'Please select exactly one item.';
      },
      theme: {
        icon: {
          checked: '(●)',
